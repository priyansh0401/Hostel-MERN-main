...
function Attendance() {
  const getALL = async () => {
    setProgress(30);
    const marked = await fetch("https://hostel-mern-main-backend.vercel.app/api/attendance/getHostelAttendance", {
      method: "POST",
      headers: {
  ...
  const markAttendance = async (id, isPresent) => {
    const data = await fetch(`https://hostel-mern-main-backend.vercel.app/api/attendance/mark`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
